    ...
    Alpine.data('pizzaCart', () => {
        return {
            ...
            loadOrderHistory() {
                ...
                const getOrderHistoryUrl = `https://pizza-api.projectcodex.net/api/pizza-cart/username/${this.username}/active`; 
                axios.get(getOrderHistoryUrl)
                    .then(result => {
                        this.orderHistory = result.data.orders || [];
                    })
                    .catch(error => {
                        console.error('There is an error fetching order history:', error);
                    });
            }
        }
    });
